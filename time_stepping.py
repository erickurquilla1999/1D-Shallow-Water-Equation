...
def compute_time_derivatives(h__, u__, bas_vals_at_gau_quad, bas_vals_x_der_at_gau_quad, gau_wei, _malla_, bas_vals_at_nod, mass_matrix_inverse__):
    ...
    numerical_flux_vector_1_, numerical_flux_vector_2_ = discontinuos_galerkin.compute_numerical_flux_vectors(bas_vals_at_nod, h__, u__)
...
def rk4_method(_h_, _u_, timestep, bas_vals_at_gau_quad_, bas_vals_x_der_at_gau_quad_, gau_wei_, malla__, bas_vals_at_nod_, mass_matrix_inverse_):
    ...
    # computing k1
    dh_dt, du_dt = compute_time_derivatives(_h_, _u_, bas_vals_at_gau_quad_, bas_vals_x_der_at_gau_quad_, gau_wei_, malla__, bas_vals_at_nod_, mass_matrix_inverse_)
    k1_h = dh_dt * timestep
    k1_u = du_dt * timestep

    # computing k2
    dh_dt, du_dt = compute_time_derivatives(_h_ + k1_h * 0.5, _u_ + k1_u * 0.5, bas_vals_at_gau_quad_, bas_vals_x_der_at_gau_quad_, gau_wei_, malla__, bas_vals_at_nod_, mass_matrix_inverse_)
    k2_h = dh_dt * timestep
    k2_u = du_dt * timestep

    # computing k3
    dh_dt, du_dt = compute_time_derivatives(_h_ + k2_h * 0.5, _u_ + k2_u * 0.5, bas_vals_at_gau_quad_, bas_vals_x_der_at_gau_quad_, gau_wei_, malla__, bas_vals_at_nod_, mass_matrix_inverse_)
    k3_h = dh_dt * timestep
    k3_u = du_dt * timestep

    # computing k4
    dh_dt, du_dt = compute_time_derivatives(_h_ + k3_h, _u_ + k3_u, bas_vals_at_gau_quad_, bas_vals_x_der_at_gau_quad_, gau_wei_, malla__, bas_vals_at_nod_, mass_matrix_inverse_)
    k4_h = dh_dt * timestep
    k4_u = du_dt * timestep
